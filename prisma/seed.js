...
const seedMockData = async () => {
    ...
    const events = await prisma.event.createMany({
        data: [
            { name: 'Concert', status: 'coming', genre: 'Music', tickets_available: 100, tickets_sold: 0, event_date: new Date('2024-12-01T20:00:00Z'), location: 'Stadium', description: 'A great music concert.' },
            { name: 'Football Match', status: 'coming', genre: 'Sports', tickets_available: 200, tickets_sold: 0, event_date: new Date('2024-12-05T18:00:00Z'), location: 'Stadium', description: 'A thrilling football match.' },
            { name: 'Art Exhibition', status: 'coming', genre: 'Art', tickets_available: 50, tickets_sold: 50, event_date: new Date('2024-11-15T10:00:00Z'), location: 'Gallery', description: 'An exhibition of modern art.' },
        ],
    });
    ...
    //         data: [
    //             { user_id: 1, event_id: 1, upload_date: new Date(), image: 'ticket_image_1.png', is_verified: true },
    //             { user_id: 1, event_id: 2, upload_date: new Date(), image: 'ticket_image_2.png', is_verified: true },
    //             { user_id: 2, event_id: 1, upload_date: new Date(), image: 'ticket_image_3.png', status: 'sold' },
    //             { user_id: 3, event_id: 3, upload_date: new Date(), image: 'ticket_image_4.png', status: 'sold' },
    //         ],
    //     });
    // } catch (e) {
    //     console.error(e)
    ...
    try {
        // Удаляем существующие рекомендации
        await prisma.recommendedEvent.deleteMany({});

        // Добавляем общие рекомендации (для всех пользователей)
        await prisma.recommendedEvent.createMany({
            data: [
                { event_id: 1, weight: 1.0 }, // Concert для всех
                { event_id: 2, weight: 1.0 }, // Football Match для всех
            ]
        });
        ...
        await prisma.recommendedEvent.createMany({
            data: [
                { event_id: 3, user_id: 1, weight: 2.0 }, // Art Exhibition специально для Alice
                { event_id: 1, user_id: 2, weight: 1.5 }, // Concert с высоким приоритетом для Bob
            ]
        });
    } catch (e) {
        console.error('Error creating recommended events:', e);
    }
};
